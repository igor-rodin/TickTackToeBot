...
while game_result == 'None':
    ...
    if x_turn: 
        print('Ходят X')
        if bot_play_X:
            dm.field = dm.field.replace(botAI.get_bot_turn(dm.field,'X',bot_algorithm),'X') # бот сходил за X
        else:    
            dm.field = dm.field.replace(dm.MyInput(dm.field),'X') # чел сходил за X
    else: 
        print('Ходят O')
        if not bot_play_X:
            dm.field = dm.field.replace(botAI.get_bot_turn(dm.field,'O',bot_algorithm),'O') # бот сходил за O
        else:    
            dm.field = dm.field.replace(dm.MyInput(dm.field),'O') # чел сходил за O
    game_result = dm.CheckGame(dm.field) # проверяем, чего там с результатом игры
    x_turn = not x_turn
