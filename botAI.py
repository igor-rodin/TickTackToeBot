...
def get_bot_turn(local_field:str,my_char,algorithm): # щаг бота - в зависимости от алгоритма
    match algorithm:
        case 2: return my_random(local_field)          # случайный 
        case 3: return mega_brain(local_field,my_char) # самый умный
        case _: return first_free(local_field)         # самый тупой
...
def win_triple(triple:str,my_char): # triple - строка из 3-х символов (цифры и Х и О)
    ...
    if len(s) == 1 and s.isdigit() : return s # если одно место с числом осталось - то это то, что нам нужно
    return ''

def i_can_win(local_field,my_char): # поверка, можем ли мы победить одним ходом, когда играем за my_char
...
def mega_brain(local_field:str,my_char):
    ret = i_can_win(local_field,my_char) # проверяем, можем ли победить одним ходом
    if ret: return ret # можем!

    if my_char == 'X': alien_char = 'O'
    else: alien_char = 'X'

    ret = i_can_win(local_field,alien_char) # проверяем, может ли враг победить одним ходом
    if ret: return ret # может! - ходим туда - ему назло!

    best_move = ['5', '1', '3', '7', '9', '2', '4', '6', '8'] # лучшие ходы на поле
